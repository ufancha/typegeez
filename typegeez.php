...
// i18n
// As of WordPress 4.6, plugins hosted on WordPress.org do not need to call
// load_plugin_textdomain() manually. WordPress will automatically load
// translations from the global languages directory for the plugin slug.
//
// If you distribute this plugin outside of WordPress.org and want to load
// the bundled .mo files from the plugin's /languages directory, you can
...
function typegeez_register_assets(){
  ...
  // Fall back to the plugin file modification time if asset mtime can't be determined.
  $fallback_ver = @filemtime(__FILE__);
  wp_register_style('typegeez-style', $plugin_url . 'assets/style.css', array(), $ver_css ? $ver_css : $fallback_ver);
  wp_register_script('typegeez-translit', $plugin_url . 'assets/translit.js', array(), $ver_translit ? $ver_translit : $fallback_ver, true);
  wp_register_script('typegeez-main', $plugin_url . 'assets/main.js', array('typegeez-translit'), $ver_main ? $ver_main : $fallback_ver, true);
}
...
function typegeez_register_block(){
  $plugin_url = plugin_dir_url(__FILE__);
  $plugin_path = plugin_dir_path(__FILE__);
  $ver_block = @filemtime($plugin_path . 'assets/block-editor.js');
  $fallback_ver = @filemtime(__FILE__);
   wp_register_script('typegeez-block-editor', $plugin_url . 'assets/block-editor.js', array('wp-blocks','wp-element'), $ver_block ? $ver_block : $fallback_ver, true);
   // Server-side render the block using the dedicated render callback
   register_block_type('typegeez/editor', array(
    'editor_script' => 'typegeez-block-editor',
    'render_callback' => 'typegeez_render_block'
  ));
// shortcode remains `typegeez_editor`
add_shortcode('typegeez_editor', 'typegeez_render_block');
}
...
function typegeez_enqueue_block_editor_assets(){
  ...
  $ver_translit = @filemtime($plugin_path . 'assets/translit.js');
  $ver_main = @filemtime($plugin_path . 'assets/main.js');
  $ver_gutenberg = @filemtime($plugin_path . 'assets/gutenberg-typing.js');
  $fallback_ver = @filemtime(__FILE__);
  wp_enqueue_script('typegeez-translit', $plugin_url . 'assets/translit.js', array(), $ver_translit ? $ver_translit : $fallback_ver, true);
  wp_enqueue_script('typegeez-main', $plugin_url . 'assets/main.js', array('typegeez-translit'), $ver_main ? $ver_main : $fallback_ver, true);
  ...
  wp_enqueue_script(
    ...
    $ver_gutenberg ? $ver_gutenberg : $fallback_ver,
    true
  );
  $ver_style = @filemtime($plugin_path . 'assets/style.css');
  wp_enqueue_style('typegeez-style', $plugin_url . 'assets/style.css', array(), $ver_style ? $ver_style : $fallback_ver);
}
...
function typegeez_enqueue_admin_assets($hook){
  ...
  $plugin_path = plugin_dir_path(__FILE__);
  $ver_translit = @filemtime($plugin_path . 'assets/translit.js');
  $ver_admin = @filemtime($plugin_path . 'assets/admin-editor.js');
  $ver_style = @filemtime($plugin_path . 'assets/style.css');
  $fallback_ver = @filemtime(__FILE__);
  wp_enqueue_script('typegeez-translit', $plugin_url . 'assets/translit.js', array(), $ver_translit ? $ver_translit : $fallback_ver, true);
  wp_enqueue_script('typegeez-admin-editor', $plugin_url . 'assets/admin-editor.js', array('jquery','typegeez-translit'), $ver_admin ? $ver_admin : $fallback_ver, true);
